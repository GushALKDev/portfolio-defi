...
class Token extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (gmaTokenData) {
      const gmaToken = new web3.eth.Contract(
        GMAToken.abi,
        gmaTokenData.address
      );
      this.setState({ gmaToken: gmaToken });
      let gmaTokenBalance = await gmaToken.methods
  ...
  _getVariables = async () => {
    try {
      const _GMATokensBalance = await this.state.gmaToken.methods
        .balanceOf(this.state.account)
        .call();
      this.setState({
        GMATokensBalance:
          window.web3.utils.fromWei(_GMATokensBalance, "ether") + " GMA",
      });
      const _RWDTokensBalance = await this.state.rewardToken.methods
        .balanceOf(this.state.account)
        .call();
      this.setState({
        RWDTokensBalance:
          window.web3.utils.fromWei(_RWDTokensBalance, "ether").slice(0, -10) +
          " RWD",
      });
    } catch (err) {
  ...
  _buyTokens = async (_amount) => {
    try {
      ...
      const tokensPrice = _amount * 0.001;
      ...
      Swal.fire({
        ...
        width: 800,
        padding: "3em",
        text: `You bought ${_amount} Tokens by ${(ethers / 10 ** 18).toFixed(
          2
        )} ETH`,
        backdrop: `
          rgba(15, 238, 168, 0.2)
  ...
  _returnTokens = async (_amount) => {
    try {
      ...
      Swal.fire({
        ...
        width: 800,
        padding: "3em",
        text: `You returned ${_amount} Tokens by ${(ethers / 10 ** 18).toFixed(
          2
        )} ETH`,
        backdrop: `
          rgba(15, 238, 168, 0.2)
      ...
      return (
        <div>
          ...
          <div className="loading">
            ...
            />
            <p className="mx3 my-0">
              Loading blockchain data, wait a moment please...
            </p>
          </div>
        </div>
      );
